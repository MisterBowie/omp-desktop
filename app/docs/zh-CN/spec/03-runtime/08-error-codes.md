...
会话由哪个 Agent 运行时执行，是桌面在任何工作开始**之前**就决定的边界（规格 02 §4，M2/T08）。
当某个会话的引擎无法提供所请求的能力时，以这些代码拒绝；**绝不**改用另一个引擎的路径执行。

| 代码 | 可重试 | 含义 |
|---|---|---|
| `ENGINE_CAPABILITY_UNAVAILABLE` | 否 | 该会话的引擎在当前构建中不支持所请求的能力（其能力声明为关闭，或运行时未启动）。携带 `engine` 与 `capability`；消息只说明引擎，不含任何回退 |
| `ENGINE_UNAVAILABLE` | 是 | 引擎运行时未在运行或启动失败；运行时恢复可用后可重试（协议、版本或进程失败） |
