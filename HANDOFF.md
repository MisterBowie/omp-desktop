...
- 状态：**已完成返修；R3 仍为硬阻塞**（分支 `codex/m5-omp-transition-hooks`，基线 `ba7806c0646180b2b40a83fa998f2437295174af`，首轮产出 `a78cec6df4572031365e8ea6a4237ca8db04ab20`，返修轮产出见本分支最新提交）。目标是解除 T20-A 记录的 R3 阻塞；**复审 F3 判定该目标在本层不可达**，故补丁降级为风险收敛，T20-B 不得开始。
...
- 已交付（风险收敛）：`app/patches/oh-my-pi/0001-rpc-host-tool-transition-contract.patch`（10 文件，sha256 `378c3d6b…`，72885 字节）+ `manifest.json`（含 `status: not-strict`、阻塞原因与已撤回声明）+ `app/scripts/omp-patch.mjs`（manifest/schemaVersion/checksum/base 校验、canonical 目标解析与父级符号链接拒绝、安全 scratch、`--prepare-build`、`--verify` 进程组回收、finally 清理）+ `apps/desktop/test/omp-patch.test.mjs`（17 项）+ 双轨 spike。
...
- 验证（返修轮实跑）：未补丁 spike **38/46（失败恰为 8 项 R3）**；patched spike **46/46**（**仅覆盖 loop 可控路径，不构成 R3 解除依据**）；补丁树改动面套件 **171/171**（agent-loop 143 + rpc-host-tools 13 + rpc-input-frame 15）；脚本测试 **17/17**；`node scripts/omp-patch.mjs --check --json` 与 `--apply --prepare-build --verify` 通过；agent typecheck 0 / coding-agent 仅预存在错误；oxlint/oxfmt/biome 0（新增脚本不在 biome include 面内，`node --check` 兜底）；`git diff --check ba7806c..HEAD` 与 `git show --check HEAD` 均 exit 0（`.patch` artifact 作用域 whitespace 豁免）；T20 定向 37/37；矩阵 lint 0；gap 诊断仍 3 缺口 exit 1；两个子模块 SHA 与工作树未动、无残留进程/scratch。
- 限制（如实记录）：补丁树内 `packages/agent` 全量 `bun test` 本轮未重跑（既有夹具 `delayed-tool-mcp.ts` 曾挂起；本轮已实现并单测验证按进程组回收，但**未声称全量套件通过**）。
