...
- 验证（返修轮实跑）：未补丁 spike **38/46（失败恰为 8 项 R3）**；patched spike **46/46**（**仅覆盖 loop 可控路径，不构成 R3 解除依据**）；补丁树改动面套件 **171/171**（agent-loop 143 + rpc-host-tools 13 + rpc-input-frame 15，补丁未改动故未重跑）；脚本测试 **24/24**（F6-F9：canonical 别名与 source 围栏、退化 pid 守卫、flag 取值边界、spawn 失败快速结算；返修前同套件 18/24 的 RED 已记录；**macOS 本机 24/24 待复审确认**）；`node scripts/omp-patch.mjs --check --json` 与 `--apply`（含符号链接父级拒绝冒烟）通过；agent typecheck 0 / coding-agent 仅预存在错误；oxlint/oxfmt/biome 0；`git diff --check ba7806c..HEAD` 与 `git show --check HEAD` 均 exit 0（`.patch` artifact 作用域 whitespace 豁免）；T20 定向 37/37；矩阵 lint 0；gap 诊断仍 3 缺口 exit 1；两个子模块 SHA 与工作树未动、无残留进程/scratch。
- 限制（如实记录）：补丁树内 `packages/agent` 全量 `bun test` 本轮未重跑（既有夹具 `delayed-tool-mcp.ts` 曾挂起；本轮已实现并单测验证按进程组回收，但**未声称全量套件通过**）；F6 的 macOS 端到端验收（同一脚本套件 24/24）**必须在 macOS 本机复审确认**，本机 Linux 只提供策略/遍历单测、真实根别名（`/bin`、`/lib`）用例与旧策略对照探针；`child.on("error")` 在"子进程已存在、pid 有效"时的整组杀灭是防御性分支，**未测、不声称**。
