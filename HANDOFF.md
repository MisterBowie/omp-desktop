...
- 验证：`@pi-desktop/omp-runtime` vitest 289/289、desktop 定向 139/139、真实固定 OMP E2E 17/17（host-tool 6 + capability 5 + session/concurrent/persistence/subagent 6）、desktop 全量 2831 总数/2827 通过/0 失败/4 跳过、typecheck 2×0、build 0、style-token lint 0、Biome 0、`git diff --check` 0、secret 扫描 0、子模块固定 SHA、无残留进程/临时目录。
