...
- 全量回归：`@pi-desktop/omp-runtime` **239 通过 / 0 失败**（本轮 +12 B3 回归）、`@pi-desktop/shared` 968 通过、desktop 2687 通过/0 失败/4 跳过、`pnpm typecheck`/`build:js`/`git diff --check` 通过。
