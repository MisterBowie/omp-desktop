...
更新时间：2026-09-25。当前状态：**M5/T20 受阻**——T20-A（Plan/Goal 与高权限工具能力门：能力审计、基线红灯诊断与最小实现契约）已完成并经**三轮复审返修 + 一轮纯文档澄清**（R1-R4、F1-F7、F8-F12、F13/F14；**R3 为硬阻塞**；分支 `codex/m5-plan-goal-capability-gates`；坐标：审计基线 `2ca256520980aa80bb812730fbad132e9e95f503` → T20-A 初稿 `07de0a725269486c909d36bcccb9d8bb209770de` → R1-R4 `5da616f04c1ec132954785b955c1b20cc17bfc54` → F1-F7 `dd2ec72cbe2dc2d338ce20467da390bb1110601e` → F8-F12 `25e0d4d254ff641df1840cfe2cd767a7e9f00eb1`，证据 `docs/validation/M5-plan-goal-capability-gates.md`；固定子模块 OMP `d49918fab`、PI `0111e306` 未动）；**T20-B/C/D 未开始，T20 未完成、未声称，且 T20-B 在 R3 解除前不得开始**。此前 **M5/T19（MCP、规则、技能、记忆及插件分类适配）已完成**：T19-A（运行时能力源隔离、可信网关加载、能力所有权契约；分支 `codex/m5-capability-sources`，基线 `ff4c1efed63475b9f552ace8a3f28f69b317e176`）与 T19-B（host-tool RPC：桌面/插件 MCP 与插件 Agent Tools 经单一宿主边界恰一次暴露；分支 `codex/m5-host-tool-rpc`，基线 `93ee82b61bd8191722db199e0fb8f1c74ddfaa12`，证据 `docs/validation/M5-host-tool-rpc.md`）；T19-C（桌面技能/记忆/插件用户路径；分支 `codex/m5-capability-user-paths`，基线 `df49b84f8246a83c4ed5cb1e2a3e0c72ee88e538`，证据 `docs/validation/M5-capability-user-paths.md`）。T18 已通过独立复审并验收（验收产品基线 `a35da2f87a745fbedd8f5beb46368607c2f6b72e`，分支 `codex/m5-tool-results`；证据见 `docs/validation/M5-tool-results.md`）。T17 已验收基线 `ab07a888d6afb916561b80e5661ed27aa6be612a` 保持不动。下一阶段入口：**无（M5/T20-B 受阻）**——R3 硬阻塞解除前不得开始 T20-B；解除条件见 `docs/validation/M5-plan-goal-capability-gates.md` §6.3/§8（上游 OMP 给 host tool 加 `concurrency`/`terminate` 或等价批次可见钩子、或用户明确接受语义差异、或把提交移出模型循环）。T19-A 证据见 `docs/validation/M5-capability-sources.md`；T19-B 证据见 `docs/validation/M5-host-tool-rpc.md`；T19-C 证据见 `docs/validation/M5-capability-user-paths.md`；T18 证据见 `docs/validation/M5-tool-results.md`；T17 历轮独立复审返修见 `docs/validation/M5-subagents.md` §0.1-§0.15。
