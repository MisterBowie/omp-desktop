...
更新时间：2026-09-23。当前状态：**M5/T17（OMP 子代理生命周期、父子归属、进度/详情查看、取消边界与恢复语义）实现完成并已提交**（分支 `codex/m5-subagents`，见下方 §0.5 摘要与 `docs/validation/M5-subagents.md`），已通过两轮独立复审返修（R1-R8 与 S1-S4）。接入路径沿用 M1 定案；M5 把子代理三类帧接入既有 Pi 拓扑/详情/工作面板 UI，单独立停止保持关闭（固定 OMP 无 per-child stop RPC）。下一阶段 M5/T18-T20（edit/LSP/DAP 展示、MCP/规则/技能/记忆、Plan/Goal 能力门）。

## 0. M5/T17 交付摘要（本轮）

- 状态：**T17 已完成并提交**（分支 `codex/m5-subagents`），含真实固定 OMP 子代理端到端验收，已按独立复审两轮返修并追加提交：R1-R8（父 stop 泄漏、渲染器接线、结算破坏 Task 行、所有权 fail-closed、读取上界、快照补漏、订阅失败、文档）与 S1-S4（父 stop 漏 lifecycle 仍查快照并保留未回收所有权、渲染器增量累积/reset 与单飞、existing child 缺父拒绝、toolResult 有界投影）。
