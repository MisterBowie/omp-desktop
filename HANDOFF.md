...
更新时间：2026-09-25。当前状态：**M5/T20 进行中（T20-R3A 补丁轮已完成，R3 底层阻塞解除，T20-B 可以开始但未开始）**——T20-A（Plan/Goal 与高权限工具能力门：能力审计、基线红灯诊断与最小实现契约）已完成并经**三轮复审返修 + 一轮纯文档澄清**（R1-R4、F1-F7、F8-F12、F13/F14；分支 `codex/m5-plan-goal-capability-gates`；坐标：审计基线 `2ca256520980aa80bb812730fbad132e9e95f503` → T20-A 初稿 `07de0a725269486c909d36bcccb9d8bb209770de` → R1-R4 `5da616f04c1ec132954785b955c1b20cc17bfc54` → F1-F7 `dd2ec72cbe2dc2d338ce20467da390bb1110601e` → F8-F12 `25e0d4d254ff641df1840cfe2cd767a7e9f00eb1`，证据 `docs/validation/M5-plan-goal-capability-gates.md`）→ **T20-R3A「OMP 过渡工具契约补丁」已完成**（分支 `codex/m5-omp-transition-hooks`，基线 `ba7806c0646180b2b40a83fa998f2437295174af`，patch level **`d49918f+omp-desktop.1`**：未补丁 spike 仍 38/46、patched **46/46**，补丁树改动面套件 164/164，补丁脚本 `--check`/`--apply --prepare-build --verify` 与 12 项负向测试全绿，证据 `docs/validation/M5-omp-transition-patch.md`、ADR 0305；**这是本项目维护的 patch level，不是官方 OMP 18.2.7 原生支持**）；固定子模块 OMP `d49918fab`、PI `0111e306` 未动；**T20-B/C/D 未开始，T20 未完成、未声称**。

## 0. M5/T20-R3A 交付摘要（本轮）

- 状态：**已完成并验证**（分支 `codex/m5-omp-transition-hooks`，基线 `ba7806c0646180b2b40a83fa998f2437295174af`）。目标：只解除 T20-A 记录的 **R3 硬阻塞**（OMP 固定运行时缺少「过渡工具独占批次」与「提交后终止」能力），不做桌面 Plan/Goal UI/模式状态/审批闭环，不开始 T20-B/C/D。
- 证据：`docs/validation/M5-omp-transition-patch.md`（PI 契约复核、上游最新 main `ea8b542` 仍缺能力的逐项实测、补丁内容与 A-E 契约映射、patch/未补丁双轨计数、脚本安全性与负向测试、符号链接踩坑、限制）；设计决策：`app/docs/adr/0305-pinned-runtime-patch-set.md` + `app/docs/spec/03-runtime/02-agent-runtime.md` §15。
- 交付物：`app/patches/oh-my-pi/0001-rpc-host-tool-transition-contract.patch`（10 文件，sha256 `4351025d…`，52556 字节）+ `manifest.json`（base SHA `d49918f…`、版本 18.2.7、checksum、4 个能力标识）+ `app/scripts/omp-patch.mjs`（唯一应用入口：校验 → 安全 scratch → 应用 → 可选 `--prepare-build`/`--verify` → finally 清理）+ `apps/desktop/test/omp-patch.test.mjs`（12 项）+ 双轨 spike。
- 契约（通用、不含任何桌面产品/工具名）：`RpcHostToolDefinition.concurrency?: "shared" | "exclusive"`（省略 = shared，非法值整请求拒绝）；`batchPolicy?: "any" | "sole"`（**整批**拒绝：零执行、零 `tool_execution_start`、零 host call、按调用顺序 blocked 结果、不触发审批钩子）；`AgentToolResult.terminate`/`AfterToolCallResult.terminate`（批次结算后结束 run，成功与错误同样，非 abort、不改写 stopReason、steering 保留）；`host_tool_result.result.terminate` 透传（顶层 `isError` 组合为 `frame.isError || result.isError`，partial 永不终止）。
- 验证：未补丁 spike **38/46（失败恰为 8 项 R3）**、patched spike **46/46**；补丁树 `agent-loop`+`rpc-host-tools`+`rpc-input-frame` **164/164**；agent 包 typecheck/lint/format 干净（coding-agent typecheck 仅一处预存在 `judgment-chain.test.ts` fetch 类型错误，在未改动固定检出上逐字复现）；`node scripts/omp-patch.mjs --check` 通过、`--apply --prepare-build --verify` 9.6 秒内 launcher 报 18.2.7 且树内套件通过；父仓库 T20 定向 **37/37**、矩阵 lint 0、gap 诊断仍 3 缺口 exit 1（未静态假绿）、biome 0、`node --check`/`git diff --check` 0；两个子模块 SHA 与工作树未动、无残留进程/scratch。
- 限制（如实记录）：补丁树的 agent 包**全量** `bun test` 未跑完——卡在既有夹具 `delayed-tool-mcp.ts` 的子进程上（6 分钟后中止并终止其进程组），**未声称全量通过**；已改为以改动面套件 + typecheck/lint 作为通过证据。
- 下一轮入口：**M5/T20-B**（模式状态机、提示词、目录、提交/审批/派发闭环）可以开始（R3 三条解除条件均已满足），但本轮未开始；仍关闭：`branch`/`steer`/`followUp`/`compact`、子代理单独停止、子代理 `hasUI=false` 工具 gating、PI agent 扩展注入。
