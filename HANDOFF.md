...
更新时间：2026-09-26。当前状态：**M5/T20 受阻（R3 仍为硬阻塞，T20-B 不得开始；本轮 T20-R3C 按用户决定交付「Plan/Goal 不支持 Cursor 模型」的产品门，并经独立复审返修 F1-F4：不变量已下沉到 host-core 的持久化写入门）**——T20-A（Plan/Goal 与高权限工具能力门：能力审计、基线红灯诊断与最小实现契约）已完成并经**三轮复审返修 + 一轮纯文档澄清**（R1-R4、F1-F7、F8-F12、F13/F14；分支 `codex/m5-plan-goal-capability-gates`；坐标：审计基线 `2ca256520980aa80bb812730fbad132e9e95f503` → T20-A 初稿 `07de0a725269486c909d36bcccb9d8bb209770de` → R1-R4 `5da616f04c1ec132954785b955c1b20cc17bfc54` → F1-F7 `dd2ec72cbe2dc2d338ce20467da390bb1110601e` → F8-F12 `25e0d4d254ff641df1840cfe2cd767a7e9f00eb1`，证据 `docs/validation/M5-plan-goal-capability-gates.md`）→ **T20-R3A「OMP 过渡工具契约补丁」已完成返修**（分支 `codex/m5-omp-transition-hooks`，基线 `ba7806c0646180b2b40a83fa998f2437295174af`，patch level **`d49918f+omp-desktop.1`**）：补丁提供 loop 层收敛（批次计数对齐 PI 的全部 toolCall、声明 sole 工具的会话关闭工具投机、整批拒绝、terminate 透传、RPC 边界严格校验）与可复现的 patch/应用脚本入口，**但复审 F3 判定严格契约在本层不可实现**——OMP 的 `kCursorExecResolved` 调用在 assistant message 形成前已由 provider/editor bridge 执行（`block-symbols.ts:46-57`、`agent.ts:1491-1497`），其副作用不可撤销，任何 loop 层实现都无法保证「整批零副作用」。**故 R3 仍是硬阻塞，T20-B 不得开始；上一轮「R3 已解除 / T20-B 可以开始」的声明已撤回**（证据与撤回声明：`docs/validation/M5-omp-transition-patch.md`、ADR 0305）。固定子模块 OMP `d49918fab`、PI `0111e306` 未动；**T20-B/C/D 未开始，T20 未完成、未声称**。

## 0. M5/T20-R3C 交付摘要（Plan/Goal × Cursor 产品门，本轮）

- 状态：**已实现并验证（只交付用户批准的产品门）**。分支 `codex/m5-cursor-plan-goal-gate`，基线 `eaf014fec34ce7780ab67ff110d17df61a479974`（本阶段为其上的追加提交）。用户选择产品选项 1：**Plan/Goal 模式不支持 Cursor 模型**；本轮把它落成唯一不变量「**一个会话不得把活跃的 Cursor 模型/提供方与 Plan 或 Goal 模式组合**」。**严格 R3 仍不受支持（0/12），T20-B/C/D 未开始、T20 未完成**；不改动 host-core、两个子模块与 patch artifact/manifest。
- 证据：`docs/validation/M5-cursor-plan-goal-gate.md`（PI 先/OMP 后证据、可达状态矩阵、实现、测试、限制与预存在缺陷）；设计：`app/docs/adr/0306-plan-goal-excludes-cursor-models.md` + spec `03-runtime/08-error-codes.md` §3.2b、`03-runtime/02-agent-runtime.md` §16。
- 先例结论（先说事实）：**PI Desktop 与固定 OMP 都没有"模式×模型互斥"先例**——PI 的模式门只判状态/上下文（`PLAN_ALREADY_ACTIVE`/`PLAN_CONFIGURATION_BLOCKED`/`PLAN_REQUIRES_INTERACTIVE_SESSION`），模型门只判配置/认证（`MODEL_NOT_CONFIGURED`）；OMP 自身有 plan/goal 状态与只读写保护，但 `RpcCommand`/`RpcSessionState` 无模式面，也没有"因模式拒绝模型"的代码。因此这是一条**新概念**，由 ADR 明确论证后才落地。
- 可达性（决定"门是否只是摆设"）：本产品**当前无法表达 Cursor 传输**——`API_STYLES` 无 cursor 传输（`packages/shared/src/model-catalog.ts:22-30`）、OMP 投影只映射 5 种 HTTP api 且拒绝 OAuth（`omp-model-projection.ts:19-36,108,116-118`）、Pi 侧 `pi-ai@0.86.1` 中 `cursor-agent` 出现 **0** 次；provider 行 id 是 UUID 或插件命名空间（`crates/host-core/src/providers/repository.rs:87`、`crates/host-core/src/plugins/providers.rs:71`）。所以今天唯一可达的 Cursor 身份是**持久化的 `provider_id = "cursor"`**（如导入的 pi/opencode 会话，导入强制 `mode:"agent"`）。门在今天已生效，并在 Cursor 将来可投影时自动继续生效。
- 实现（复审返修后）：**主强制点在 host-core 的持久化写入本身**——`crates/host-core/src/plan_goal_guard.rs` 在每次打开数据库时安装两个 `sessions` 触发器（`INSERT`、`UPDATE OF mode, provider_id`），凡结果落入 Plan/Goal × Cursor 的写入由 SQLite 中止；覆盖 `plans.enter`（sidecar 直连 host-core）、`session.configure`、`session.create`、`session.fork`、`session.import` 与任何将来写入者，条件按数据库当前行在写入时求值（无"预读—写入"窗口）。旧无效记录仅当 mode/provider **完全不变**时可写（`plan+cursor → goal+cursor` 被拒），两条修复路径保留，桌面派发门为迁移/手工数据的最后防线。稳定错误码 `PLAN_GOAL_CURSOR_UNSUPPORTED` 由 host RPC（`plans.enter`/`session.configure`/`session.create`/`session.fork`/`session.import`）与桌面三边界（`sessionConfigure`/`sessionCreate`/`agentPrompt`）统一返回；桌面拒绝经统一构造函数携带 `data = { errorCode, mode, providerId }`，由 `register.ts` 作为 `error.details` 抵达渲染层。共享谓词 `planGoalCursorRefusal`（身份＝规范 provider id `cursor` 的**精确**比较）供桌面/渲染层使用；跨语言字面量一致性由 `plan-goal-cursor-constant-parity` 钉住，守卫 SQL 由 Rust 常量生成。渲染层＝store 先于 IPC 拒绝并弹本地化提示、模型菜单在契约模式隐藏 Cursor 提供方并说明原因、会形成组合的自动 pin 跳过并提示。**绝不自动改写模式或模型**：两条出路（切到 Agent / 换模型）由用户选择；8 个 locale 同步文案。
- 测试（先红后绿）：**host-core 未修时 4 项判红**（`plans.enter` 返回 `Ok(())`、`session.configure`/`session.create` 落盘 `mode:"plan"+provider_id:"cursor"`、`plans.enter` RPC 返回 `{"ok":true,"state":"planning"}`），修后 `cargo test -p host-core` **596/0**（新增 14 项：守卫 6、enter 2、sessions 5、RPC 1）。桌面新增/更新 20 项（跨语言一致性 3、真实 wrapper/IPC 载荷 3、行为测试 10、渲染层契约 4），另有 shared 谓词 7 项。失效性对照：去掉桌面门 → 6/10 判红；把 UPDATE 容忍条件改回过宽形式 → 3 项旧记录测试全红；移除共享 `data` → 2 个文件全红。实跑：desktop 全量 **2907 项 / 2903 通过 / 0 失败 / 4 跳过**、shared src 491/491（官方命令 982 含 dist 编译副本）、i18n 25/25、`pnpm typecheck` 全包 0、`pnpm build:js` exit 0、`pnpm lint` 0、`error-code-registry` 2/2、`check-locales` 79 对、`check-docs` 仅 6 项预存在问题、`OMP_T20_GAP_PROBE=1` 仍 3 缺口 exit 1、`check-t20-matrix-ids` 0、`git diff --check` 与 `git show --check` 0；三处转译夹具补上新的 shared 依赖（断言未改）。
- 如实记录的限制：①严格 R3 未解、OMP 的 Plan/Goal 运行时面未实现（G1/G2/G3 仍开放，本阶段不实现、不声称）；②**旧数据的边界（能力事实，非不变量缺口）**：守卫只约束写入，由迁移或手工编辑在守卫之前产生的行仍留在库中——它们不能被继续写入该组合、也不能在契约模式之间切换，但记录本身仍在，直到用户走修复路径，其**运行**由桌面派发门拒绝；"打开即自动修复/拒绝打开"属另一项产品决策，本阶段未做；③**预存在缺陷（未修）**：`app/docs/scripts/check-docs.mjs` 在起点提交即报 6 项（`adr/0301` 的 H1 不以 `ADR` 开头 + `adr/0301`–`0305` 缺 `adr/README.md` 索引行），本轮只为自己的 ADR 0306 补索引行；④未做 UI E2E（`verify:ui:*` 需用户显式要求）、未做 macOS 实机复核、未重跑 Rust 套件（未改 Rust）。
- 下一轮入口：**无（受阻）**；仍关闭：`branch`/`steer`/`followUp`/`compact`、子代理单独停止、子代理 `hasUI=false` 工具 gating、PI agent 扩展注入。

## 0. M5/T20-R3A 交付摘要（补丁轮，含独立复审返修）

- 状态：**已完成返修；R3 仍为硬阻塞**（分支 `codex/m5-omp-transition-hooks`，基线 `ba7806c0646180b2b40a83fa998f2437295174af`，首轮产出 `a78cec6df4572031365e8ea6a4237ca8db04ab20`，第一轮返修 `474408de2b2bdb538f5b2249d71398b8537e1fed`，F6-F9 第二轮返修产出见本分支最新提交）。目标是解除 T20-A 记录的 R3 阻塞；**复审 F3 判定该目标在本层不可达**，故补丁降级为风险收敛，T20-B 不得开始。
- 证据：`docs/validation/M5-omp-transition-patch.md`（§1 结论与撤回、§2 F3 的 PI/OMP 源码依据、§3 已实施的收敛、§5 全部命令与计数、§6 边界安全修复、§8 今后解除 R3 的可选路径）；设计决策：`app/docs/adr/0305-pinned-runtime-patch-set.md`（含 Unresolved (F3) 段）+ `app/docs/spec/03-runtime/02-agent-runtime.md` §15（标题即标注“R3 not lifted”）。
- F3 依据（精确位置）：PI 对 assistant message 的**全部** `toolCall` 计数（`upstream/pi-desktop/packages/agent-runtime/src/runtime.ts:2222-2234`）；OMP 的 `kCursorExecResolved` 块由 Cursor exec channel 在流式期间**已执行**、结果缓冲（`packages/ai/src/utils/block-symbols.ts:46-57`、`packages/agent/src/agent.ts:1491-1497`、`packages/agent/src/agent-loop.ts:1493-1496`）；工具投机同样在流式期间启动物理执行（`packages/agent/src/speculative-execution.ts:847-905`，由 `agent-loop.ts:2264` 的 `toolcall_end` 触发）。因此“含 sole 工具的批次零副作用”不可满足，且**不得**把口径缩小为“仅 loop dispatch 的调用”。
- 已交付（风险收敛）：`app/patches/oh-my-pi/0001-rpc-host-tool-transition-contract.patch`（10 文件，sha256 `e08ca7ff…fd8`，72978 字节；F6-F9 轮复核对齐 manifest）+ `manifest.json`（含 `status: not-strict`、阻塞原因与已撤回声明）+ `app/scripts/omp-patch.mjs`（manifest/schemaVersion/checksum/base 校验、**逐组件 canonical 解析**——仅跟随 root 直属系统别名如 macOS `/var`、其余父级符号链接拒绝、source/仓库根/cwd canonical 保护边界、flag 取值边界、安全 scratch、`--prepare-build`、`--verify` 进程组回收 + 退化 pid 防护、finally 清理）+ `apps/desktop/test/omp-patch.test.mjs`（**24 项**）+ 双轨 spike。
- 补丁新增/修正的契约：批次计数=消息内全部 toolCall（与 PI 同口径，Cursor 已执行调用也计入并导致整批拒绝）；声明 sole 工具的 provider 调用**关闭工具投机**（代价：该会话失去 read/eval 投机预执行）；sole 批次整批拒绝（不触发审批钩子）；`AgentToolResult.terminate`/`afterToolCall` override（结算后终止、非 abort）；`host_tool_result.result.terminate` 必须为可选 boolean（非 boolean 拒绝，不按真值强转）；`concurrency`/`batchPolicy` 仅 `undefined` 取默认（`null` 与非法值整请求拒绝、不留半注册）。
- 验证（返修轮实跑）：未补丁 spike **38/46（失败恰为 8 项 R3）**；patched spike **46/46**（**仅覆盖 loop 可控路径，不构成 R3 解除依据**）；补丁树改动面套件 **171/171**（agent-loop 143 + rpc-host-tools 13 + rpc-input-frame 15，补丁未改动故未重跑）；脚本测试 **24/24**（F6-F9：canonical 别名与 source 围栏、退化 pid 守卫、flag 取值边界、spawn 失败快速结算；返修前同套件 18/24 的 RED 已记录；macOS 本机 **24/24（连续两次）** 已由独立复审在 `47d0258` 实跑确认）；`node scripts/omp-patch.mjs --check --json` 与 `--apply`（含符号链接父级拒绝冒烟）通过；agent typecheck 0 / coding-agent 仅预存在错误；oxlint/oxfmt/biome 0；`git diff --check ba7806c..HEAD` 与 `git show --check HEAD` 均 exit 0（`.patch` artifact 作用域 whitespace 豁免）；T20 定向 37/37；矩阵 lint 0；gap 诊断仍 3 缺口 exit 1；两个子模块 SHA 与工作树未动、无残留进程/scratch。
- 限制（如实记录）：补丁树内 `packages/agent` 全量 `bun test` 本轮未重跑（既有夹具 `delayed-tool-mcp.ts` 曾挂起；本轮已实现并单测验证按进程组回收，但**未声称全量套件通过**）；macOS 端到端验收已由独立复审在本机确认（同一脚本 suite 在 `47d0258` 上**连续两次 24/24**、exit 0，含 F7 最小真实 git fixture 与 `--check --json` 复核，见证据文档 §5/§7；本机 Linux 亦为 24/24 连续两次，另有策略/遍历单测、真实根别名（`/bin`、`/lib`）用例与旧策略对照探针）；`child.on("error")` 在"子进程已存在、pid 有效"时的整组杀灭是防御性分支，**未测、不声称**。
- 下一轮入口：**无（受阻）**——解除 R3 需要上游改动、产品层显式禁用 Cursor exec 通道（含 provider/bridge 开关）或用户明确接受语义差异（见证据文档 §8）；仍关闭：`branch`/`steer`/`followUp`/`compact`、子代理单独停止、子代理 `hasUI=false` 工具 gating、PI agent 扩展注入。

## 0. M5/T20-A 交付摘要（本轮）

- 状态：**T20-A 审计与契约已完成三轮返修（R1-R4、F1-F7、F8-F12）+ 一轮纯文档澄清（F13/F14），但"无硬阻塞"结论已撤回**（分支 `codex/m5-plan-goal-capability-gates`；**提交坐标**：审计基线 `2ca2565` → T20-A 初稿 `07de0a7` → R1-R4 返修 `5da616f` → F1-F7 返修 `dd2ec72` → F8-F12 返修 `25e0d4d`，即每一轮的基线是上一轮的产出提交）。最近一轮实验轮为 **F8-F12（基线 `dd2ec72`，产出 `25e0d4d`）**，其后的 F13/F14 只改文档：未实现任何生产 Plan/Goal 行为、未改 UI/host 架构、**T20 未完成、未声称**；固定子模块零改动。
- 证据：`docs/validation/M5-plan-goal-capability-gates.md`（§1.3.1 完整有效权限决策表；§4 **保守 g1 判据（永不自动判 closed）**的 opt-in 诊断 + 假绿对照实验；§4.2 固定 OMP + fake provider 的可行性 spike；§5 差距表含 G8/G9；§6 最小设计；§7 验收矩阵 B1-B14/C1-C8/D1-D3（编号唯一性由脚本校验）；§8 阻塞）。
- 核心结论（F8-F12 返修后）：
  - **F8 已修（保守方案）**：静态符号不能证明数据流 → g1 **永不自动判 closed**；基线 `GAP-OPEN`，任何 mode-ish state 字段 / composer 接缝 / 协议键只报 `REVIEW-REQUIRED` 并计入退出码，退场只能由 T20-B 的 B2/B13 行为测试替换。对照：仅注释写关键词 → `REVIEW-REQUIRED`（exit 1）；仅加 state 字段 → `REVIEW-REQUIRED`；复原 → `GAP-OPEN`。脚本头部"disjunction"与文档"真实链路"措辞已删除。
  - **F9 已修**：R4-4 第三个 prompt 改为真实变化（catalog 加 `SubmitGoal`、clamp 去 `HostEcho`），实测 attempts **2/1/2**，三 prompt 工具表与 clamp 严格相等。
  - **F10 已修**：systemBytes 改用 `Buffer.byteLength(..., "utf8")` 并新增 `prefixBytes = system − block` 断言；实测 blockBytes 176/1726/2156、systemBytes 14608/16158/16592、prefixBytes 14432/14432/14436（工具目录变化使第三个前缀 +4 字节，块仍是纯追加）。
  - **F11 已修**：R4-3 改称"目标模式目录序列 / 模拟的 catalog 生命周期"，明确不证明真实持久模式切换（仍属 B1/B13 的 T20-B 验收）；同 session 连续 prompt、增删重加、无滞留结论保留。
  - **F12 已修**：权限矩阵外部路径行注明仅适用于已通过契约 allowlist 的工具，Plan/Goal 非许可工具仍先 Deny。
- 核心结论（F1-F7 返修轮）：
  ...
  - **F2 已修（历史）**：当时把 g1 改为"四段链路同时成立"并观察到注入完整链路会 `GAP-CLOSED`；**该静态判据已被 F8 推翻并删除——现状以 F8 为准**（g1 永不自动判 closed，只由 T20-B 的 B2/B13 行为测试替换退场）。保留的历史事实：裸 `mode` 字段不构成证据、协议加键走 `REVIEW-REQUIRED`。
  ...
  - **F5 已修**：spike 追加**生产 `composeModeSystemPrompt(mode, "")` 的三个真实块**并断言 UTF-8 字节/次数/位置（`system = prefix + block`）；技能/记忆块内容与顺序明确留给 T20-B 的 B2 ④。
  ...
  - **F13/F14（纯文档，本轮）**：统一 g1 的最终口径——所有历史摘要中"四段链路可判 `GAP-CLOSED`"的表述都已标注被 F8 推翻并删除，当前契约只有"永不自动 closed + `REVIEW-REQUIRED` + 由 B2/B13 替换退场"；并统一四轮提交坐标（初稿 `07de0a7` / R1-R4 `5da616f` / F1-F7 `dd2ec72` / F8-F12 `25e0d4d`，每轮基线为上一轮产出），不再把某轮产出当作"返修基线"来引用。
...
- 验证：诊断默认退出 0（SKIP）/ opt-in 退出 1（3 缺口 + g4）；g1 假绿对照两项（注释关键词、仅加字段，均 `REVIEW-REQUIRED`，无任何静态分支可判绿）；矩阵编号脚本 0；spike `t20-feasibility.mjs` **38/46**（失败恰为 R3 契约 8 项；attempts 2/1/2、blockBytes 176/1726/2156、systemBytes 14608/16158/16592、prefixBytes 14432/14432/14436）；37 项定向测试 37/37；Biome 0；`git diff --check` 0；固定 OMP 局部测试 5+3+3 通过；字节平价 `diff -q` 四文件全等；两个子模块 SHA 未动、工作树干净。
- 下一轮入口：**无（受阻）**；仍关闭：`branch`/`steer`/`followUp`/`compact`、子代理单独停止、子代理 `hasUI=false` 工具 gating、PI agent 扩展注入。

## 0. M5/T19-C 交付摘要（上一轮，含第二轮复审返修）

- 状态：**T19-C 已实现并验证**（分支 `codex/m5-capability-user-paths`，首轮基线 `df49b84`；复审返修追加提交基于 `10d2366`）。桌面技能目录与项目记忆经可信 gate 的 `before_agent_start` 每 prompt 注入 OMP 系统提示（追加、绝不替换原生 prompt/规则/上下文/原生技能目录），正文经按需 `Skill` host tool 实时读取（PI 优先级与错误形状）；T20 未实现、未声称。
- 证据：`docs/validation/M5-capability-user-paths.md`（固定源码证据、纯基线 df49b84 严格红灯、返修轮红/绿见 §3.3/§8、绿测计数、限制）；设计决策：ADR 0304 §5 追加 + spec `03-runtime/02-agent-runtime.md` §14 追加。
- 核心变更：
  ...
  - **运行域状态文件**（`packages/omp-runtime/src/desktop-state.ts` + supervisor）：`OmpRunPaths.desktopState`（`<runRoot>/desktop-state.json`）；supervisor 在 buildOmpRuntimeEnv 之后设 `OMP_DESKTOP_STATE`（静态参数/`extraEnv` 无法改道）；bridge 每 prompt 前**原子替换**（同目录唯一临时文件 + `wx` + 0600 + `rename` 换入 + finally 清理；final path 符号链接/硬链接作为条目被替换、绝不跟随；失败保留旧文件且不留临时文件）；随 run root 清理。
  - **gate 注入**（`omp-desktop-gate.ts`）：`before_agent_start` 读取并校验（常规文件/大小/依赖无关显式字段校验/新鲜度 ≤10 min/**拒绝未来时间戳**/属主 session 匹配；子代理零注入），返回 `{systemPrompt:[...event.systemPrompt, block]}`；block 与 PI `pluginSkillsPrompt`/`projectMemoryPrompt` 逐字节一致（跨包 parity 测试钉死）；任何失败 → 无 override、turn 以原生 prompt 继续、审批门不受影响。解析器依赖无关（gate 在 OMP 进程内加载，包导入会解析到运行时模块图）。
  - **按需 `Skill` host tool**：目录非空才注册（PI gate）；executor 按 **builtin → user(调用时按 `loadUserSkillBody(id, projectPath)` 复检作用域) → plugin(仅 PI `loadSkillBody` 复检：已注册/已加载/文件/128 KiB/非空 body，无执行时 scope 谓词)** 实时读 body；plugin 作用域变化经下一 prompt 目录重建生效；成功 `# Skill: <name> (<id>)\n\n<body>`、失败 `Skill: <msg>. Available skills: <ids>.`（isError）；不是 `plugin_/mcp_` 名 → 网关不审批（PI 一致只读）。
  - **一致退化与 stale 失效（复审返修）**：host 读失败 = PI best-effort（无 memory 块/仅少 user 技能）；snapshot 抛错、状态写入失败或 fresh state 自校验失败 → **空 tombstone 原子替换旧状态**（gate 只能读到「无注入」）并撤回该 turn 的 Skill 工具；**tombstone 写不进 → prompt 提交前失败（`OMP_CAPABILITY_STATE_FAILED`）**，绝不带着 stale 目录/记忆提交；**写入成功后以与 gate 相同的 `readDesktopCapabilityState` 契约回读并核对属主 session，`skillsPresent` 只由校验过的状态决定**；失败日志仅 presence-only 分类（`kind` + `hasCode` 布尔），绝不输出 message/stack/name/code 值或正文/凭据。
- 新增测试：`desktop-state.test.ts`（21）、`desktop-state-supervisor.test.ts`（4）、`gate-desktop-state.test.ts`（7）；`omp-desktop-capabilities.test.mjs`（6，含 PI parity）、`omp-skill-path-bridge.test.mjs`（9）、`omp-skill-path-adapter.test.mjs`（10）、`omp-skill-path-e2e.test.mjs`（6 真实固定 OMP）。
- 先红后绿：首轮全部探针文件 + 纯基线 df49b84 红（vitest 3 文件全 fail、capabilities 5/5、bridge 5/5、adapter 7/9〔2 guard 基线即过〕、E2E 6/6，清理经 `t.after` 账本零残留）；**复审返修轮分两批红**——residual 1-3 于未修 `10d2366`（vitest 3 fail：植目录拒绝/未来时间戳读取/未来时间戳注入；bridge 3 fail：首轮写失败契约、tombstone 失效、tombstone 不可写 fail-before-prompt）；residual 6-7 于其修复后的工作树（bridge 2 fail：gate 会拒绝的 fresh state 仍注册 Skill、日志携带 message 哨兵；residual 7 收紧后 name/code/errorCode 哨兵亦红）；adapter scope guard 基线即通过=当前实现与固定 PI 已一致。
- 验证：`@pi-desktop/omp-runtime` vitest 322/322、desktop 定向 177/177、T19-C E2E 6/6、真实固定 OMP 既有六套件回归 17/17（合并跑 23/23）、desktop 全量 2863 总数/2859 通过/0 失败/4 跳过、typecheck 2×0、`pnpm build:js` 0、style-token lint 0、Biome 0、`git diff --check` 0、子模块固定 SHA、无残留进程/临时目录。
...
- 核心变更：
  ...
  - **执行**（omp-runtime `host-tools.ts` `OmpHostToolCalls` + 桌面 executor）：`host_tool_call` 绑定当前 run；at-most-once 覆盖 owned generation 内的**执行**（按 generation 无限记录、结束回收，无 LRU 驱逐）；fail-closed 拒绝执行数为 0，重放被拒帧再次 fail-closed 应答；未知 targetId/迟到 result/cancel 无副作用；`host_tool_cancel` 只按 `targetId` 关联、不进转换器；stop/abort/dispose/传输失败/run close 取消全部 pending、迟到完成丢弃；执行前复检插件加载/作用域/MCP 活性/最新工具表/最后同步派发点 `signal.aborted` + `dispatchable(turnId)`（OMP runner 实时状态，不混用 Pi `activeTurns`）；binding 的 modelKey/thinkingLevel 为执行时 getter（configure thinking-only 原位生效）。执行上下文固定 `mode:"agent"`：PI `host.ts` 传持久会话 mode、`plugin-runtime.ts:2505-2525` 依 `ctx.mode` 执行 planSafeActions，OMP 路径当前无 mode——T20 传播并执行真实 mode 前不声称 Plan/Goal 或完整 PI 执行上下文兼容。
  - **审批**：既有 trusted gate 无条件受控 `plugin_*`/`mcp_*`（`OMP_DESKTOP_GATE_TOOLS` 只调原生名），副作用前真实 `tool_permission_request`；risk：plugin_=high（保守上界，协议无声明风险通道，低/中风险插件提示更严格——如实记录）、mcp_=medium（**更正：PI 侧 `mcp_*` 是 `Risk::Low`；此处的 medium 是桌面 gate 当时更严格的选择，不是 PI parity。完整决策表与 T20 的修正要求见 `docs/validation/M5-plan-goal-capability-gates.md` §1.3.1/G9/§6.10**）；无并行审批系统。
  ...
  - **结果保真**：两层 isError（抛错与内层 `{content,isError:true}`）→ 外层 `host_tool_result.isError`；text/image 保真透传（MCP image 与 OMP `ImageContent` 同形）；audio/resource/structuredContent/未知键（含插件返回的 details/providerMetadata/useless）确定性文本化、不因旁有 text 丢失、裸 base64 不跨线、绝不映射到 OMP 结果自身字段；所有 outcome 统一过共享的 `boundHostToolContent`（runtime 包）按最终 `JSON.stringify(content)` 字节（≤768 KiB）判定/二分截断、标记计入预算、超限保留结果头部；同一函数是 `OmpHostToolCalls` 的最终权威写入边界，执行器抛出的超大错误同样受限并保留外层 `isError: true`。
  - **toast**：生产 adapter 执行后按 PI 顺序 drain+emit，异常隔离，不影响工具结果。
  - **MCP 取消边界**：客户端无调用级 signal 且 `UserMcpRuntime.callTool` 派发前有连接握手 await——只承诺「进入 MCP 调用路径前已观察到的取消会拒绝该调用；进入后连接/请求可能继续，只能丢弃迟到完成」，不声称阻止或撤回远端副作用（固定 PI 客户端同此限制）。中止边界按固定源码细分：child-backed plugin Agent Tool 经 `ctx.signal` 真实中止（`sendToChild` abort）；plugin-declared MCP 注册闭包忽略 `ctx.signal`（`plugin-runtime.ts:3506-3519`），与 MCP 同受无调用级取消限制、只能丢弃迟到完成。
- 新增测试：`host-tools.test.ts`（17：at-most-once 跨 5000 调用无驱逐、closeGeneration 回收、cancel/迟到完成、两层 isError、malformed、fail-closed 重放契约、超大抛错/超大 outcome 写入边界、非文本溢出保留头部）、`turn-end.test.ts`（9：completed/aborted/error 各路径恰一次、converter 错误不误报、无 dialog prompt 失败仍公告、抛错公告不阻塞 close）、`host-tool-runner.test.ts`（9：帧路由/去重/取消/停止/传输/处置）、`omp-host-tool-bridge.test.mjs`（16：注册顺序/回显/拒绝/目录增删/每会话隔离/turn-end reason/execute gate/live thinking）、`omp-host-tool-adapter.test.mjs`（29：目录保真/空白名拒绝/作用域/重名/dispatch gate/signal/两层 isError/image 透传/structuredContent/details 文本化/Unicode 与转义预算/头部保留/toast 隔离）、`omp-host-tool-e2e.test.mjs`（6 真实固定 OMP：三类工具审批先到→allow 恰一次→结果专属 canary 进入 `role:"tool"` 模型消息、deny 零执行零副作用、取消无迟到副作用、decoy 不被原生发现；全部经生产 adapter）。
...
- 验证：`@pi-desktop/omp-runtime` vitest 290/290、desktop 定向 140/140、真实固定 OMP E2E 17/17（host-tool 6 + capability 5 + session/concurrent/persistence/subagent 6）、desktop 全量 2832 总数/2828 通过/0 失败/4 跳过、typecheck 2×0、build 0、style-token lint 0、Biome 0、`git diff --check` 0、secret 扫描 0、子模块固定 SHA、无残留进程/临时目录。
