...
更新时间：2026-09-25。当前状态：**M5/T20-A（Plan/Goal 与高权限工具能力门：能力审计、基线红灯诊断与最小实现契约）已完成**（分支 `codex/m5-plan-goal-capability-gates`，基线 `2ca256520980aa80bb812730fbad132e9e95f503`，证据 `docs/validation/M5-plan-goal-capability-gates.md`；固定子模块 OMP `d49918fab`、PI `0111e306` 未动）；**T20-B/C/D 待开始，T20 未完成、未声称**。此前 **M5/T19（MCP、规则、技能、记忆及插件分类适配）已完成**：T19-A（运行时能力源隔离、可信网关加载、能力所有权契约；分支 `codex/m5-capability-sources`，基线 `ff4c1efed63475b9f552ace8a3f28f69b317e176`）与 T19-B（host-tool RPC：桌面/插件 MCP 与插件 Agent Tools 经单一宿主边界恰一次暴露；分支 `codex/m5-host-tool-rpc`，基线 `93ee82b61bd8191722db199e0fb8f1c74ddfaa12`，证据 `docs/validation/M5-host-tool-rpc.md`）；T19-C（桌面技能/记忆/插件用户路径；分支 `codex/m5-capability-user-paths`，基线 `df49b84f8246a83c4ed5cb1e2a3e0c72ee88e538`，证据 `docs/validation/M5-capability-user-paths.md`）。T18 已通过独立复审并验收（验收产品基线 `a35da2f87a745fbedd8f5beb46368607c2f6b72e`，分支 `codex/m5-tool-results`；证据见 `docs/validation/M5-tool-results.md`）。T17 已验收基线 `ab07a888d6afb916561b80e5661ed27aa6be612a` 保持不动。下一阶段入口：M5/T20-B（按 `docs/validation/M5-plan-goal-capability-gates.md` §6-§7 契约实现）。T19-A 证据见 `docs/validation/M5-capability-sources.md`；T19-B 证据见 `docs/validation/M5-host-tool-rpc.md`；T19-C 证据见 `docs/validation/M5-capability-user-paths.md`；T18 证据见 `docs/validation/M5-tool-results.md`；T17 历轮独立复审返修见 `docs/validation/M5-subagents.md` §0.1-§0.15。

## 0. M5/T20-A 交付摘要（本轮）

- 状态：**T20-A 已实现并验证**（分支 `codex/m5-plan-goal-capability-gates`，基线 `2ca2565`）。本轮是**能力审计 + 可执行基线红灯诊断 + 最小实现契约**，未实现任何生产 Plan/Goal 行为、未改 UI/host 架构、**T20 未完成、未声称**；固定子模块零改动。
- 证据：`docs/validation/M5-plan-goal-capability-gates.md`（PI 契约以固定 `upstream/pi-desktop` 为主证据并逐文件 diff 核实 fork 差异；OMP 能力以固定 `upstream/oh-my-pi` 为证据；rpc-ui 判定与 interactive/ACP 分离；基线红灯命令与输出；差距表；最小设计；验收矩阵 B/C/D 归属；阻塞与未知）。
- 核心结论：
  - **PI 契约**：Plan/Goal 共用一种 kind 参数化管线——`sessions.mode` 持久、`composeModeSystemPrompt` 提示词、`plan_mode_allows`（Read/Glob/Grep/Bash/BrowserPreview/new_context）目录 + host-core 契约模式硬拒绝（Write/Edit/无 planSafeActions 插件/未知一律 Deny，先于一切权限模式）、`SubmitPlan/SubmitGoal` → 不可变 `.pi/<kind>/*.md` 产物 + pending 行 + `awaiting_approval`、approve 原子写 mode=agent+permission_mode+queued 执行、claim CAS 先于执行、boot 重启打断不重放、**Bash 无命令白名单（由 permissionMode 决定，提示词要求只读但硬边界是权限模式）**、插件 `planSafeActions` 逐 action 把关且真实模式经 host-core 持久模式传播（边车 mode 字段被忽略）。
  - **OMP 判定**：rpc-ui = rpc + 工具 UI 上下文，42 个命令全列无任何 plan/goal/permission/approval 命令；OMP 自身 Plan/Goal 是 interactive/ACP 专属（proposal handler、TUI overlay、elicitation、goal 定时器），rpc-ui 不可复用。可行通道三条（均无需改上游）：`before_agent_start` systemPrompt 追加、`setActiveTools` 目录过滤、`tool_call` 执行前 fail-closed 拦截（toolCallId/toolName/input + ctx hasUI/sessionManager）。
  - **基线红灯**：新增 `app/scripts/check-omp-plan-goal-gaps.mjs`（opt-in `OMP_T20_GAP_PROBE=1` 诊断，不进默认套件）：无 env 打印 SKIP 退出 0；有 env 输出 g1（mode/permissionMode 仅持久化）/g2（插件执行硬编码 agent 模式）/g3（已批准计划执行对 OMP 会话被拒、永不 claim）三条 `GAP-OPEN` 与 g4（rpc-ui 命令面 42 变体全列、无 plan/goal 命令）判定，退出 1。
  - **最小设计**（validation §6）：复用 host-core plans 模块/`PlanApprovalBar`/`composeModeSystemPrompt`（字节平价，不手抄）、T19-C 状态文件通道传模式提示词与 per-turn 权限策略、`SubmitPlan/SubmitGoal` 注册为桌面 host tool 调既有 `plans.submit`、gate 按 PI `permissions.rs` 语义执行时强制（含 inherit 解析；**明确不宣称现有 `OMP_DESKTOP_GATE_MODE`/OMP `--approval-mode` 已提供权限模式映射**）、claim/drain/boot_maintenance 原样复用（恰一次、不重放）。无硬阻塞，T20-B 可开始。
- 验收矩阵：B1-B10（T20-B）、C1-C5（T20-C）、D1-D3（T20-D），每行含层/期望/先例（validation §7）。
- 验证：`node scripts/check-omp-plan-goal-gaps.mjs` 退出 0（SKIP）；`OMP_T20_GAP_PROBE=1 …` 退出 1（3 缺口 + rpc-ui 判定，输出见 validation §4）；Biome 0；`git diff --check` 0；固定子模块 SHA 未动、OMP launcher 报 `omp/18.2.7`（`bun install --frozen-lockfile` + `bun run build:native` 已备）。
- 下一轮入口：M5/T20-B（模式状态机 + 提交/审批闭环 + OMP 派发）；仍关闭：`branch`/`steer`/`followUp`/`compact`、子代理单独停止、子代理 `hasUI=false` 工具 gating、PI agent 扩展注入。
