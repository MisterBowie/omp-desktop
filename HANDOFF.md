...
更新时间：2026-09-24。当前状态：**M5/T17（OMP 子代理生命周期、父子归属、进度/详情查看、取消边界与恢复语义）未验收**（分支 `codex/m5-subagents`，见下方 §0.5 摘要与 `docs/validation/M5-subagents.md`）。前两轮独立复审返修（R1-R8 与 S1-S4）已提交；**第三轮独立复审拆出 B1-B3 三项返修，本提交仅实现 B1（回收未完成时的重试，`docs/validation/M5-subagents.md` §0.3），B2（renderer 单飞/错误可见）与 B3（总 UTF-8 预算）仍未解决，T17 不验收。**接入路径沿用 M1 定案；M5 把子代理三类帧接入既有 Pi 拓扑/详情/工作面板 UI，单独立停止保持关闭（固定 OMP 无 per-child stop RPC）。下一阶段 M5/T18-T20（edit/LSP/DAP 展示、MCP/规则/技能/记忆、Plan/Goal 能力门）。

## 0. M5/T17 交付摘要（本轮）

- 状态：**T17 未验收、进行中**（分支 `codex/m5-subagents`），含真实固定 OMP 子代理端到端验收，已按独立复审两轮返修并追加提交：R1-R8（父 stop 泄漏、渲染器接线、结算破坏 Task 行、所有权 fail-closed、读取上界、快照补漏、订阅失败、文档）与 S1-S4（父 stop 漏 lifecycle 仍查快照并保留未回收所有权、渲染器增量累积/reset 与单飞、existing child 缺父拒绝、toolResult 有界投影）。第三轮复审返修：B1（回收未完成时的重试）已实现待复审，B2（renderer 单飞/错误可见）与 B3（总 UTF-8 预算）待实现。
- 证据：`docs/validation/M5-subagents.md`（PI 实现/测试 → 固定 OMP 能力 → 本项目决策证据表、归属矩阵、用户路径、失败/取消矩阵、命令与计数、限制；顶部 §0.1/§0.2/§0.3 为返修记录）；设计决策：`app/docs/adr/0303-omp-subagent-surfacing.md`（英文 ADR）。
- 核心变更：
...
- 新增/改动测试：`subagent-frames.test.ts`(16)、`subagents.test.ts`、`subagent-runner.test.ts`（含 B1 `pending reclaim retry` 5 例：首败二成后第三停 `nothing running`、连续失败、teardown 抛错、`reaped:true/cleaned:false` 目录债、并发单飞）、`omp-subagent-bridge.test.mjs`(8)、`omp-subagent-e2e.test.mjs`(3 真实固定 OMP，含父 stop 回收运行中 detached 子代理，且不再预先 `listSubagents` 预热)、`subagent-reload-projection.test.mjs`(R3)、`omp-subagent-read.test.mjs`(R2/S2 真实 api + merge 去重)、`omp-subagent-panel-render.test.mjs`(R2/S2 SSR 分流 + loading/empty/error/retry 可见)、`omp-subagent-panel-mounted.test.mjs`(S2 真实 mounted renderer：挂载 hook、运行 effects、打桩 `window.piDesktop.invoke`、断言真实 `api.ts` channel)。
