...
- 实现：共享谓词 `planGoalCursorRefusal(mode, providerId)`（`packages/shared/src/plan-goal-model-gate.ts`；身份＝规范 provider id `cursor` 的**精确**比较，不做名称/供应商/URL 猜测）+ 新错误码 `PLAN_GOAL_CURSOR_UNSUPPORTED`。强制点＝`sessionConfigure`（按**有效对**判定，覆盖"进入 Plan/Goal"与"在 Plan/Goal 下切换模型"两向）、`sessionCreate`、`agentPrompt`（陈旧/导入组合，先于任何运行时工作）。渲染层＝store 先于 IPC 拒绝并弹本地化提示、模型菜单在契约模式**隐藏** Cursor 提供方并说明原因、会导致组合的自动模型 pin 跳过并提示。**绝不自动改写模式或模型**：两条出路（切到 Agent / 换模型）由用户选择；8 个 locale 同步文案。
- 测试：共享谓词边界表 6 项、真实 IPC 处理器行为测试 10 项（含两条出路与非 Cursor 回归）、渲染层契约 4 项；**失效性对照：去掉门后 10 项行为测试中 6 项判红**（prompt 门 2 项、session 门 4 项），4 项回归/出路断言在两种状态下都必须为绿。实跑：shared 490/490、i18n 25/25、desktop 全量 **2901 项 / 2897 通过 / 0 失败 / 4 跳过**、真实固定 OMP 子代理 E2E 3/3、desktop typecheck 0、`pnpm build:js` 成功、`pnpm lint` 0、`error-code-registry` 2/2、`OMP_T20_GAP_PROBE=1` 仍 3 缺口 exit 1、`check-t20-matrix-ids` 0、`git diff --check` 与 `git show --check` 0；三处转译夹具补上新的 shared 依赖（断言未改）。
- 如实记录的限制：①严格 R3 未解、OMP 的 Plan/Goal 运行时面未实现（G1/G2/G3 仍开放，本阶段不实现、不声称）；②**一条桌面不可拦截的 Pi 路径**——provider 绑定无法解析时启动解析会回退到默认/首个提供方（`packages/host-runtime/src/launch-resolver.ts:227-231`），模型可经 `plans.enter`（sidecar 直连 host-core，`crates/host-core/src/plans/approval.rs:129-165`）写入 `mode='plan'`；该组合由**派发门**确定性拒绝（持久记录可落入组合，但任何后续回合都不会在其中运行）；若要求持久记录本身永不落入组合，落点是 host-core `PlanManager::enter` + 跨语言字面量等价测试；③**预存在缺陷（未修）**：`app/docs/scripts/check-docs.mjs` 在起点提交即报 6 项（`adr/0301` 的 H1 不以 `ADR` 开头 + `adr/0301`–`0305` 缺 `adr/README.md` 索引行），本轮只为自己的 ADR 0306 补索引行；④未做 UI E2E（`verify:ui:*` 需用户显式要求）、未做 macOS 实机复核、未重跑 Rust 套件（未改 Rust）。
