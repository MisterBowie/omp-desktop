...
更新时间：2026-09-24。当前状态：**M5/T17（OMP 子代理生命周期、父子归属、进度/详情查看、取消边界与恢复语义）未验收**（分支 `codex/m5-subagents`，见下方 §0.5 摘要与 `docs/validation/M5-subagents.md`）。前两轮独立复审返修（R1-R8 与 S1-S4）已提交；第三轮独立复审拆出 B1-B3，B1（回收未完成时的重试，§0.3）与 B1 残余缺陷 F1/F2（§0.4）已实现；**第四轮独立复审拆出 C1（回收成功后的运行时替换/原生会话恢复，§0.5），C1 基本续聊已修复，但其三项残余缺陷（R1 替换后回复覆盖、R2 停止未完成即替换且跳过恢复、R3 启动/恢复中停止被忽略）由本跟进提交修复（§0.6），待独立验收；B2（renderer 单飞/错误可见）与 B3（总 UTF-8 预算）仍未解决，T17 不验收。**接入路径沿用 M1 定案；M5 把子代理三类帧接入既有 Pi 拓扑/详情/工作面板 UI，单独立停止保持关闭（固定 OMP 无 per-child stop RPC）。下一阶段 M5/T18-T20（edit/LSP/DAP 展示、MCP/规则/技能/记忆、Plan/Goal 能力门）。

## 0. M5/T17 交付摘要（本轮）

- 状态：**T17 未验收、进行中**（分支 `codex/m5-subagents`），含真实固定 OMP 子代理端到端验收，已按独立复审两轮返修并追加提交：R1-R8（父 stop 泄漏、渲染器接线、结算破坏 Task 行、所有权 fail-closed、读取上界、快照补漏、订阅失败、文档）与 S1-S4（父 stop 漏 lifecycle 仍查快照并保留未回收所有权、渲染器增量累积/reset 与单飞、existing child 缺父拒绝、toolResult 有界投影）。第三轮复审返修：B1（回收未完成时的重试）与 B1 残余缺陷 F1/F2 已实现；**第四轮复审返修：C1（回收成功后的运行时替换/原生会话恢复）基本续聊已修复，其三项残余缺陷（替换后回复覆盖、停止未完成即替换且跳过恢复、启动/恢复中停止被忽略）由本跟进提交修复（§0.6）待复审**；B2（renderer 单飞/错误可见）与 B3（总 UTF-8 预算）待实现。
