...
| **G8** | **过渡工具独占批次与回合终止：仍为硬阻塞（T20-R3A 复审确认）**——补丁已把批次计数对齐 PI（全部 toolCall）、在声明 sole 工具的会话关闭工具投机、并提供批次拒绝与结算终止，但 Cursor exec channel 在 assistant message 形成前已执行调用（`block-symbols.ts:46-57`、`agent.ts:1491-1497`），其副作用无法在 loop 层阻止/撤销，故"整批零副作用"不可满足 | `docs/validation/M5-omp-transition-patch.md` §2/§3；`app/patches/oh-my-pi/manifest.json` status | **T20-B 不得开始**；需上游改动、产品层禁用该通道或用户明确接受语义差异 |
...
> 状态更新（T20-R3A 复审）：R3 **仍为硬阻塞**（§8）。补丁轮提供的 loop 层收敛（全 toolCall 计数、sole 会话关闭投机、批次拒绝、terminate 透传）不能覆盖 Cursor exec channel 的预执行副作用，故 B 表中依赖 G8 的行仍**不得开始**；先前"已解除"的表述已撤回。
...
- **硬阻塞（R3，T20-R3A 复审后仍成立）**：固定 OMP 18.2.7 上**无法**实现 PI 的过渡工具契约——(a) 过渡工具独占 assistant 批次、同批其它调用零副作用；(b) 提交成功或失败后不再继续模型循环；(c) 不依赖结果文本或模型服从。T20-R3A 曾以 patch level `d49918f+omp-desktop.1` 尝试解除，但独立复审 F3 确认：PI 对 assistant message 的**全部** `toolCall` 计数（`runtime.ts:2222-2234`），而 OMP 的 `kCursorExecResolved` 调用在消息形成前已由 provider/editor bridge 执行且结果缓冲（`packages/ai/src/utils/block-symbols.ts:46-57`、`packages/agent/src/agent.ts:1491-1497`），其副作用**不可能**被 loop 层阻止或撤销；工具投机也会在流式期间启动物理执行（`packages/agent/src/speculative-execution.ts:847-905`）。R3A 已实施的收敛（全 toolCall 计数、sole 会话关闭投机、批次拒绝、terminate 透传）只覆盖 loop 可控路径，**不足以解除 R3**，且**不改变验收口径**（不得缩小为"仅 loop dispatch 的调用"）。证据与撤回声明：`docs/validation/M5-omp-transition-patch.md` §1-§4。**T20-B 不得开始。**
...
**T20-A 的审计、契约与验收矩阵已完成三轮返修（R1-R4、F1-F7、F8-F12），并由 F13/F14 纯文档轮统一了 g1 最终口径与提交坐标（不改变任何结论）。T20-R3A（补丁轮）尝试以本项目维护的 patch level 提供缺失能力，但独立复审 F3 判定严格契约在本层不可实现（Cursor exec channel 在 assistant message 形成前已执行调用，副作用不可撤销），**故 R3 仍是硬阻塞（§8），T20-B 不得开始**；R3A 的补丁降级为风险收敛。** T20-B/C/D 未开始；Plan/Goal 与高权限工具门未实现、未声称完成。F1-F7（风险表/验收、g1 判据、严格序列、目录生命周期、mode block 字节与范围、矩阵唯一性、行号）与 **F8-F12**（g1 改为永不自动 closed 的保守判据 + 注释假绿对照、R4-4 真实变化、UTF-8 字节与前缀不变量、R4-3 证据范围收窄为目录序列、外部路径行限定 allowlist）已按固定源码与无费用实验修正；T20-R3A 的补丁轮**没有**解除 R3：复审判定严格契约在本层不可实现（Cursor exec channel 在消息形成前已执行、投机可预执行），补丁降级为风险收敛，R3 仍为硬阻塞，T20-B 不得开始；见 `docs/validation/M5-omp-transition-patch.md`。`branch`/`steer`/`followUp`/`compact`/子代理单独停止保持关闭（T17-T19 延续）。
