...
| **G8** | ~~过渡工具独占批次与回合终止不可实现（硬阻塞）~~ **已由 patch level `d49918f+omp-desktop.1` 解除（T20-R3A）**：批次准入（`batchPolicy: "sole"`，整批零执行/零 `tool_execution_start`/零 host call、顺序稳定 blocked 结果）与结算后终止（`AgentToolResult.terminate`，成功与错误同样、非 abort）都由本项目补丁提供并经 patched spike 46/46 验证；RPC 面 `concurrency`/批次策略声明非法值整请求拒绝 | `docs/validation/M5-omp-transition-patch.md` §3/§5；`app/patches/oh-my-pi/manifest.json` | **T20-B 可开始**；T20-B/C/D 未开始 |
...
> 状态更新（T20-R3A）：R3 的 OMP 底层能力阻塞已由 patch level `d49918f+omp-desktop.1` 解除（`docs/validation/M5-omp-transition-patch.md`），B 表中标注 ⚠ 的行不再受 G8 阻塞；T20-B 本身仍未开始。
...
| B4 | SubmitPlan/SubmitGoal 可见性与错误模式拒绝 ⚠ 曾受 G8 阻塞（现已解除） | host-core + 桥接夹具 | 仅当前 kind 的提交工具注册；`SubmitPlan` 于 Goal / 无契约时 → `PLAN_NOT_ACTIVE`/`PLAN_KIND_MISMATCH`；每会话恰一 pending（`PLAN_ALREADY_PENDING`） | `approval.rs:216-236`、`plans/tests.rs:785` |
| B5 | 提交产物：`.pi/<kind>/<slug>.md` 不可变、sha256/大小记录、防覆盖、symlink 拒；`awaiting_approval` 状态可见 ⚠ 曾受 G8 阻塞（现已解除）（触发路径） | host-core + 渲染层 | 与 PI 相同路径/内容契约；pending 行驱动审批卡 | `artifact.rs:99-152`、`plans/tests.rs:122,156,183,685` |
...
- **已解除（R3，T20-R3A 补丁轮）**：T20-A 的结论是固定 OMP 18.2.7 上**无法**实现 PI 的过渡工具契约（(a) 过渡工具独占 assistant 批次、同批其它调用零副作用；(b) 提交成功或失败后不再继续模型循环；(c) 不依赖结果文本或模型服从；§4.2 六场景实测与根因分析见历史版本），因此本轮不再依赖上游能力，而是以**本项目维护的 patch level `d49918f+omp-desktop.1`** 提供缺失契约：RPC host tool 声明 `concurrency`/批次策略、`AgentToolResult`/`afterToolCall` 的 `terminate`、`host_tool_result` 的 terminate 透传。证据：patch 46/46（未补丁基线仍 38/46、失败恰为 8 项 R3）、补丁树改动面套件 164/164、补丁脚本 `--check`/`--apply --prepare-build --verify` 与 12 项负向测试全绿，见 `docs/validation/M5-omp-transition-patch.md`。**T20-B 可以开始**（但本轮未开始）；**这不是官方 OMP 18.2.7 原生支持**，任何打包/汇报都必须带 patch level。
