...
- omp-runtime vitest **286/286**（基线 252 + host-tool-runner 9 + host-tools 13 + turn-end 9 + gate host-tool 3）。
...
- **MCP 取消**：客户端无调用级 AbortSignal；仅承诺「未派发不开始、已派发丢弃迟到结果」，不声称撤回远端副作用。plugin Agent Tool 的 signal 中止是真实的。
- **跨 generation 重放**：at-most-once 覆盖整个活跃 generation；generation 结束后帧无属主 run、fail closed 而非执行。
