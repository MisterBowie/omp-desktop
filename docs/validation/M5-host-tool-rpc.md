...
3. **审批**：host tool 经既有 trusted gate（无并行审批系统）。`isHostToolName(plugin_*/mcp_*)` **无条件受控**（`OMP_DESKTOP_GATE_TOOLS` 只调原生名，host tool 只能经 `OMP_DESKTOP_GATE_MODE` 显式放行/拒绝），副作用前产生真实 `tool_permission_request`；risk：`plugin_*` = **high（保守上界）**、`mcp_*` = medium。**协议限制**：pinned 协议无声明 risk 通道，本阶段低/中风险插件会得到高风险提示，未声称与 PI risk 完全等价（T20 不提前做）。**更正（T20-A 复审返修 R1）**：原文把 `mcp_*=medium` 写成"与 PI 一致"是错的——固定 PI `permissions.rs:139` 把 `mcp_*` 归为 `Risk::Low`，`plugin_*` 取 manifest 声明（缺失/非法 → medium）。本行的 medium 是当时桌面 gate 更严格的**桌面选择**，不是 PI parity；完整决策表见 `docs/validation/M5-plan-goal-capability-gates.md` §1.3.1，T20 的修正要求见其 G9/§6.10。
...
| `app/packages/omp-runtime/extensions/omp-desktop-gate.ts` | `isHostToolName` 无条件受控；`riskForTool` plugin_=high/mcp_=medium（**桌面选择**；PI 为 plugin 声明风险 / mcp_=Low，见 T20-A §1.3.1 更正） |
