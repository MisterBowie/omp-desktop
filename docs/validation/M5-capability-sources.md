...
表注（独立复审别名返修轮，加法提交）：定向 vitest 19/19（overlay 9 + supervisor 10）、`@pi-desktop/omp-runtime` 全量 252/252（+3：硬链接 supervisor 探针、符号链接 writer 探针、run root 符号链接替换 supervisor 探针）；desktop 定向与真实固定 OMP E2E 重跑结果、typecheck/build 均与上表一致（本轮未改动 desktop 源码）。红/绿与实现细节见 §7.7/§7.8。
...
7. **独立复审返修（overlay 写入跟随文件系统别名，`e9d727b` 之后的加法提交）**：复审复现——`prepareRun` 在 `paths.configOverlay` 放置指向 run root 之外哨兵文件的符号链接或硬链接时，`writeFileSync(path, …, "utf8")` 会跟随链接改写哨兵（本地复现：`config-overlay.yml` 为指向 `user-config.yml` 的硬链接时，哨兵被替换为 overlay YAML），与「hook 无法替换 overlay、绝不写真实用户配置」的注释与文档矛盾。修复：`writeSourceIsolationOverlay` 先 `rmSync(path, {recursive:true, force:true})` 移除占位条目本身（Node 语义：作用于条目，不跟随符号链接、不解引用硬链接；Linux x64 实测五项：symlink 移除目标保留、目录内嵌链接不被跟随、`wx` 对既有条目 EEXIST、mode 0600 生效、缺失路径不抛错），再 `writeFileSync(path, yaml, {encoding:"utf8", flag:"wx", mode:0o600})` 独占创建——移除与创建之间若再现条目则 EEXIST 失败并走统一清理；该保证覆盖 hook 在 `prepareRun` 返回前种植的任意末级条目，**不声称**防护两类并发窗口——移除与创建之间重植末级条目（`wx`/EEXIST 是该窗口唯一防护，失败走启动清理）、规范位置检查之后替换父目录（`wx` 无法察觉父目录交换）。新增探针 2 例（先红后绿）：supervisor 级硬链接探针（哨兵不变、overlay 为 nlink=1 的普通文件、含两项强制设置；`linkSync` 全平台可用）+ writer 级符号链接探针（POSIX，`it.skipIf(win32)`：Windows 建符号链接需提权/开发者模式，硬链接探针已覆盖别名类）。同步更新 supervisor 写点注释、ADR 0304 决策 2、spec §14、HANDOFF §0。验证：vitest 定向 18/18（overlay 8 + supervisor 10）、`@pi-desktop/omp-runtime` 全量 **251/251（+2）**，desktop 与 E2E/typecheck/build 计数见 §5 表注（本轮未改动 desktop 源码）。
8. **独立复审返修二（run root 被替换为符号链接仍可把写入重定向到 run root 之外，`13b3497` 之后的加法提交）**：§7.7 只防护 overlay 路径末级条目的别名；复审复现——`prepareRun` 删除 `paths.runRoot` 并把该路径替换为指向 run root 之外目录的符号链接后，`rmSync`+`wx` 写入会经父目录重定向、把 `config-overlay.yml` 创建在外部目录（本地复现：`outside/config-overlay.yml` 出现）。修复：supervisor 在 `startRuntime` 的 try 块内、`prepareRun` **之前**记录 `realpathSync(runRoot)`（规范**位置**，非 inode 身份：同词法路径换新普通目录解析结果相同、无法重定向，有意放行），`prepareRun` 之后写 overlay 之前重解析 `realpathSync(dirname(configOverlay))`，不一致或不可解析即抛错——错误与写失败共享统一 catch/`removeRunRoot`/`lastFailure`（pre-hook realpath 失败同样在 try 内，不留无主 runRoot）；统一清理的 `rmSync` 只移除被种植的条目本身、不触碰外部目录。该守卫沿用 PI-Desktop 先例（`upstream/pi-desktop/apps/desktop/electron/main/services/image-generation-service.ts:72-81`：独占 `wx` 写前 canonicalize root 与目标目录）。新增 POSIX supervisor 探针 1 例（先红后绿：外部哨兵目录与内容原样、`outside/config-overlay.yml` 不存在、`captured` 为空证明未 spawn、`status().reason === "start-failed"`、`runDirs` 空且外部目录仍在；`it.skipIf(win32)` 因父目录重定向只能以符号链接表达、Windows 建链接需提权/开发者模式——硬链接只覆盖末级条目、不覆盖父目录重定向）；位置校验的「不可解析」分支与其余 try 内错误共享既有统一清理覆盖（「overlay 写失败清理」「启动失败无残留」两例）。同步更新 supervisor/写者注释与 ADR/spec/HANDOFF 措辞（一律「规范位置」、明确不声称对任意恶意 hook 的安全）。验证：vitest 定向 19/19（overlay 9 + supervisor 10）、`@pi-desktop/omp-runtime` 全量 **252/252（+1）**，desktop 与 E2E/typecheck/build 重跑结果见 §5 表注。
