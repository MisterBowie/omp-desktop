...
| T20 | M5 | Plan/Goal 和高权限工具能力门 | T17-T19 | 不借用不适用的原权限/模式语义 | **进行中 / 受阻**（T20-A 审计与契约经复审返修：R1/R2/R4 已按固定源码与无费用 spike 修正；**R3 为硬阻塞——固定 OMP 无法实现过渡工具独占批次与提交后终止，T20-B 不得开始**；证据 `docs/validation/M5-plan-goal-capability-gates.md` §4.2/§8；T20-B/C/D 未开始，T20 未完成） |
