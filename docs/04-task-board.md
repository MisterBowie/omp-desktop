...
| T20 | M5 | Plan/Goal 和高权限工具能力门 | T17-T19 | 不借用不适用的原权限/模式语义 | **进行中 / 受阻**（T20-A 审计与契约经三轮复审返修（R1-R4、F1-F7、F8-F12）+ 一轮纯文档澄清（F13/F14），证据 `docs/validation/M5-plan-goal-capability-gates.md`；**T20-R3A 补丁轮已完成返修**：patch level `d49918f+omp-desktop.1` 提供 loop 层收敛（全 toolCall 计数、sole 会话关闭投机、批次拒绝、terminate 透传）与 patch/应用脚本入口，但复审 F3 确认 Cursor exec channel 的预执行副作用无法在 loop 层阻止/撤销，**R3 仍为硬阻塞、T20-B 不得开始**（「已解除」声明已撤回），证据 `docs/validation/M5-omp-transition-patch.md`；T20-B/C/D 未实现，T20 未完成） |
