...
当前交付阶段：M5/T20 **进行中**——T20-A（Plan/Goal 能力审计、基线红灯诊断与最小实现契约）已完成并经三轮复审返修（R1-R4、F1-F7、F8-F12）与一轮纯文档澄清（F13/F14）（分支 `codex/m5-plan-goal-capability-gates`；坐标：审计基线 `2ca2565` → T20-A 初稿 `07de0a7` → R1-R4 `5da616f` → F1-F7 `dd2ec72` → F8-F12 `25e0d4d`；证据见 `docs/validation/M5-plan-goal-capability-gates.md`）；**T20-R3A「OMP 过渡工具契约补丁」已完成（分支 `codex/m5-omp-transition-hooks`，基线 `ba7806c`，patch level `d49918f+omp-desktop.1`）：R3 的 OMP 底层能力阻塞解除——未补丁 spike 仍 38/46（8 项 R3 失败），patched spike 46/46，OMP 定向/相关单测与补丁应用脚本（含 12 项负向测试）全部通过，证据见 `docs/validation/M5-omp-transition-patch.md`；这是本项目维护的 patch level，不是上游 18.2.7 原生支持。T20-B 可以开始但尚未开始；T20-B/C/D 未实现、T20 未完成**
...
| T20 | M5 | Plan/Goal 和高权限工具能力门 | T17-T19 | 不借用不适用的原权限/模式语义 | **进行中**（T20-A 审计与契约经三轮复审返修（R1-R4、F1-F7、F8-F12）+ 一轮纯文档澄清（F13/F14），证据 `docs/validation/M5-plan-goal-capability-gates.md` §4/§4.2/§8；**T20-R3A 补丁轮已完成**：OMP 过渡工具契约以 patch level `d49918f+omp-desktop.1` 解除 R3 底层阻塞（未补丁 spike 38/46 → patched 46/46、OMP 单测与补丁脚本负向测试全绿），证据 `docs/validation/M5-omp-transition-patch.md`；**T20-B 可以开始但未开始，T20-B/C/D 未实现，T20 未完成**） |
