...
当前交付阶段：M5/T17（OMP 子代理生命周期、父子归属、进度/详情查看、取消边界与恢复语义）已通过最终独立复审并验收（验收代码基线 `ab07a888d6afb916561b80e5661ed27aa6be612a`，分支 `codex/m5-subagents`；证据见 `docs/validation/M5-subagents.md`、`app/docs/adr/0303-omp-subagent-surfacing.md`）。下一阶段入口：M5/T18-T20。历轮独立复审返修的历史记录见 `docs/validation/M5-subagents.md` §0.1-§0.14。
