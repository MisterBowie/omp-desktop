...
当前交付阶段：M5/T20 **受阻**——T20-A（Plan/Goal 能力审计、基线红灯诊断与最小实现契约）已完成并经三轮复审返修（R1-R4、F1-F7、F8-F12）与一轮纯文档澄清（F13/F14）（分支 `codex/m5-plan-goal-capability-gates`；坐标：审计基线 `2ca2565` → T20-A 初稿 `07de0a7` → R1-R4 `5da616f` → F1-F7 `dd2ec72` → F8-F12 `25e0d4d`）。**T20-R3B（provider/bridge 层可行性审计，分支 `codex/m5-r3-provider-gate`，基线 `e56979e`）已完成并返修（复审 F1-F5，`docs/validation/M5-omp-transition-patch.md` §9/§10）**：本轮用真实 Cursor exec provider 派发器 + 真实 `CursorExecHandlers` 桥接的确定性 RED 实验证明混合批次的兄弟调用在批次裁决前物理执行（**严格 R3 契约检查 12 项全部违反 = 0/12 成立**；另 3 项候选方案检查单独统计 1 成立 / 2 违反，不计入契约、不参与退出码），并逐项判定两个候选方案——**延迟 exec 到 message_end 自锁死**（`cursor.ts:949-960` 在 `done` 前 drain 所有 exec handler，上游 http2 测试 `cursor-terminal-error.test.ts:453-521` 同断言）、**全 loop 路由（关 `mcp` bridge + `cursorExternalToolExecutor`）仅非原生半边在 exec 分支一级可测（未证明 loop 接管或恰好一次）且原生帧无任何请求侧开关**（`AgentRunRequest`/`RequestContext` 字段枚举无此能力；`cursorExternalToolExecutor` 仅 auth-gateway 设置），只能"永久拒绝原生帧 = 伪造 `Tool not available`" = 产品限制。**故 R3 仍为硬阻塞，T20-B/C/D 不得开始**；未改生产代码/patch/manifest/测试/子模块（证据 `docs/validation/M5-omp-transition-patch.md` §9、`app/experiments/omp-bridge/t20-cursor-exec-order.mjs`）。**T20-R3A（OMP 过渡工具契约补丁轮）已完成返修**（分支 `codex/m5-omp-transition-hooks`，基线 `ba7806c`，patch level `d49918f+omp-desktop.1`）：补丁提供 loop 层收敛（全部 toolCall 计数、声明 sole 工具的会话关闭工具投机、批次拒绝、terminate 透传、RPC 边界严格校验）与可复现的 patch/应用脚本入口，但独立复审 **F3 判定严格契约在本层不可实现**——OMP 的 `kCursorExecResolved` 调用在 assistant message 形成前已由 provider/editor bridge 执行，副作用不可撤销，任何 loop 层实现都无法保证「整批零副作用」。**因此 R3 仍是硬阻塞，T20-B 不得开始；上一轮「R3 已解除 / T20-B 可以开始」的声明已撤回。** 第二轮独立复审返修（F6-F9，`474408de` 之上，补丁 artifact 未改动）修完脚本层的 canonical 解析（macOS 系统别名 + `--source` 保护边界）、进程组退化 pid 防护与 flag 取值边界，脚本测试 17 → **24 项全过**；第三轮只做跨平台测试修正（`81ab2ce` → `47d0258`），其后 macOS **24/24（连续两次）** 已由独立复审在本机实跑确认；**R3 结论不变**。证据：`docs/validation/M5-omp-transition-patch.md`；T20-B/C/D 未开始，T20 未完成。
...
| T19 | M5 | MCP、规则、技能、记忆及插件分类适配 | T16 | 单一加载责任、不重复工具注册 | **已完成**（T19-A：`docs/validation/M5-capability-sources.md`、ADR 0304；T19-B：`docs/validation/M5-host-tool-rpc.md`；T19-C：`docs/validation/M5-capability-user-paths.md`、ADR 0304 §5——桌面技能目录+项目记忆经可信 gate 注入、按需 `Skill` host tool 实时读、原生技能/记忆源保持关闭或原样） |
| T20 | M5 | Plan/Goal 和高权限工具能力门 | T17-T19 | 不借用不适用的原权限/模式语义 | **进行中 / 受阻**（T20-R3B provider/bridge 层审计已完成，分支 `codex/m5-r3-provider-gate`：确定性 RED 实验（真实 Cursor exec 派发器 + 真实桥接）证明混合批次兄弟调用在裁决前执行——**严格 R3 契约检查 12 项全部违反（0/12 成立）**，另有 3 项候选方案检查单独统计（1 成立 / 2 违反，不计入契约、不参与退出码）；方案 A「延迟到 message_end」自锁死、方案 B「全 loop 路由」原生帧无请求侧开关只能伪造 `Tool not available` = 产品限制；**R3 仍阻塞、T20-B/C/D 未开始**，证据 `docs/validation/M5-omp-transition-patch.md` §9/§10（含复审返修 F1-F5）。T20-A 审计与契约经三轮复审返修（R1-R4、F1-F7、F8-F12）+ 一轮纯文档澄清（F13/F14），证据 `docs/validation/M5-plan-goal-capability-gates.md`；**T20-R3A 补丁轮已完成返修**：patch level `d49918f+omp-desktop.1` 提供 loop 层收敛（全 toolCall 计数、sole 会话关闭投机、批次拒绝、terminate 透传）与 patch/应用脚本入口，但复审 F3 确认 Cursor exec channel 的预执行副作用无法在 loop 层阻止/撤销，**R3 仍为硬阻塞、T20-B 不得开始**（「已解除」声明已撤回），证据 `docs/validation/M5-omp-transition-patch.md`（脚本 suite 本机 Linux 与 macOS 均 24/24，macOS 由独立复审在 `47d0258` 实跑确认）；T20-B/C/D 未实现，T20 未完成）**；T20-R3C（Plan/Goal × Cursor 产品门）已实现并验证，并经独立复审返修（F1-F4）**：不变量「会话不得把活跃的 Cursor 模型/提供方与 Plan/Goal 组合」已下沉到 **host-core 的持久化写入本身**——`crates/host-core/src/plan_goal_guard.rs` 在打开库时安装两个 `sessions` 触发器（INSERT 与 `UPDATE OF mode, provider_id`），凡结果落入该组合的写入由 SQLite 中止，覆盖 `plans.enter`（sidecar 直连 host）、`session.configure`、`session.create`、fork、导入；旧无效记录仅当 mode/provider 完全不变时才可写（plan⇄goal 切换被拒），两条修复路径保留，派发门为迁移/手工数据的最后防线；host RPC 统一返回 `PLAN_GOAL_CURSOR_UNSUPPORTED`（不再 `INTERNAL`），桌面拒绝错误携带 `data={errorCode,mode,providerId}` 经 `wrap` 到达渲染层 `error.details`，跨语言字面量一致性由测试钉住（先红后绿：未修时 host-core 4 项判红 → 修后整套 596/0）。证据 `docs/validation/M5-cursor-plan-goal-gate.md`；ADR 0306 + spec `03-runtime/08-error-codes.md` §3.2b、`02-agent-runtime.md` §16（英文与中文镜像均已更新）。**严格 R3 仍不受支持（0/12），T20-B/C/D 仍未开始、T20 未完成** |
