...
| T18 | M5 | edit/LSP/DAP 结果与必要的专用展示 | T16 | 结构正确、未知结果可读 | 已完成（`docs/validation/M5-tool-results.md`；`tool-presentation.ts` 新增 OMP edit/LSP/debug 只读展示，LSP 诊断文本与服务器失败可见、debug snapshot/evaluation 扁平化、edit 单/多文件/rename/create/delete/no-op/partial-error/pruned；live 与 durable/子代理路径同 presenter，Pi 兼容与 4 MiB 预算不变；待最终独立复审） |
