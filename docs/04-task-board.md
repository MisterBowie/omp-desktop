...
当前交付阶段：M5/T17（OMP 子代理生命周期、父子归属、进度/详情查看、取消边界与恢复语义）实现并已提交，含真实固定 OMP 的子代理端到端验收（三帧族、子身份、`parentToolCallId`、子工具归因、deny 无副作用、allow 恰好一次、快照 reconcile、停止回收）；证据见 `docs/validation/M5-subagents.md`、`app/docs/adr/0303-omp-subagent-surfacing.md`。前两轮独立复审返修已提交（R1-R8 与 S1-S4）。**第三轮独立复审否定了 T17 已完成，拆出 B1-B3 三项返修；B1（回收未完成时的重试，§0.3）与 B1 残余缺陷（F1/F2，§0.4）已实现。第四轮独立复审在 B1 基线上拆出 C1（回收成功后的运行时替换/原生会话恢复，§0.5），C1 基本续聊已修复，其三项残余缺陷（替换后回复覆盖、停止未完成即替换且跳过恢复、启动/恢复中停止被忽略）由跟进提交 `0092572` 修复（§0.6）并获第五轮复审独立确认；第五轮进一步复现 D1/D2（dispose 期间可启动孤儿替换、启动/恢复停止期间新 prompt 逃逸 epoch，§0.7）由 `24d7270` 修复且 D1/D2 定向探针已独立通过；第六轮进一步复现 E1/E2（既有会话绕过整桥关闭、模型切换复用 live 消息/turn id，§0.8）由本提交修复并追加行为回归、已独立通过。第七轮 B2（renderer 单飞/错误可见，§0.9）已实现并追加行为回归（含真实 `SubagentPanel` 挂载回归与 test-only 夹具 canonical 化提交），独立复审确认其夹具与初始行为通过，但仍复现运行恢复续读缺陷，由本提交修复并追加回归（§0.10），B2 follow-up 已独立通过；B3（总 UTF-8 预算，§0.11）字节/Unicode 证据已独立通过；B3 follow-up 的两处行为缺陷（大思考块擦除短答案、结构化截断静默，§0.12）已修复，短答案与普通结构化标记两路径已独立通过，整字段 details 丢弃的截断指示（§0.13）已实现、已独立通过；T17 最终独立复审待验收。**上一阶段 M4 会话持久化、模型投影与并发注册表（T14-T16）实现完成，经五轮独立复审返修（R1-R9、F1-F8、G1-G3、H1-H4、J1-J5），等待复审确认。证据见 `docs/validation/M4-persistence.md`。上一阶段 当前交付阶段：M2 运行时边界与应用身份（T08-T10）实现完成，并已按独立复审两轮返修（R1-R7：生产查询接线、查询失败 fail-closed、IPC 审计、supervisor 所有权与并发、shutdown 回收日志、测试夹具可移植；S1-S6：reorder gate、先 gate 后改动持久状态、router-first 顺序、启动失败所有权交接、保留记录语义、交付物无未跟踪文件；S7：扫描回收后按 runRoot 精确释放幽灵所有权、成功 stop 清理同运行保留记录、目录债务降级不再发信号；S8：进程组回收后替换而非追加同 runRoot 记录，一次运行最多一条记录），等待复审确认。证据见 `docs/validation/M2-runtime-boundary.md`；传输决策见 `docs/decisions/001-omp-transport.md`，引擎边界决策见 `app/docs/adr/0300-engine-boundary.md`（英文）。上一阶段证据见 `docs/validation/M1-compatibility.md`。下一阶段 M5/T18-T20（edit/LSP/DAP 展示、MCP/规则/技能/记忆、Plan/Goal 能力门）。
...
| T17 | M5 | OMP 子代理面板与编排归属 | T16 | 父子事件、查看、停止、恢复边界 | 最终独立复审待验收（`docs/validation/M5-subagents.md`；ADR 0303；`subagent-frames.ts`/`subagents.ts` 严格校验与每子代理转换、`get_subagents` reconcile、`get_subagent_messages` 上界读取、`stopSubagent` 恒拒、父 stop 回收 detached 子代理、详情桥接线；`subagentEvents` 能力开放；独立复审 R1-R8、S1-S4 已提交；B1（回收未完成重试）与 F1/F2 残余缺陷已实现；第四轮 C1（回收成功后的运行时替换/原生会话恢复）基本续聊已修复，其三项残余缺陷（替换后回复覆盖、停止未完成即替换且跳过恢复、启动/恢复中停止被忽略）由 `0092572` 修复（§0.6）并获第五轮复审独立确认；第五轮 D1/D2（dispose 期间可启动孤儿替换、启动/恢复停止期间新 prompt 逃逸 epoch，§0.7）由 `24d7270` 修复且定向探针已独立通过；第六轮 E1/E2（既有会话绕过整桥关闭、模型切换复用 live 消息/turn id，§0.8）由本提交修复并追加行为回归、已独立通过；第七轮 B2（renderer 单飞/错误可见，§0.9）已实现并追加行为回归（含真实 `SubagentPanel` 挂载回归与 test-only 夹具 canonical 化提交），独立复审确认其夹具与初始行为通过，但仍复现运行恢复续读缺陷，由本提交修复并追加回归（§0.10），B2 follow-up 已独立通过；B3 总 UTF-8 预算（§0.11）字节/Unicode 证据已独立通过；B3 follow-up 的两处行为缺陷（大思考块擦除短答案、结构化截断静默，§0.12）已修复，短答案与普通结构化标记两路径已独立通过，整字段 details 丢弃的截断指示（§0.13）已实现、已独立通过；夹具可移植性返修（§0.14，macOS `/proc` 缺失 + 封闭 PATH：`process.execPath`+shell 引号、`realpathSync` canonical 夹具根、跨平台 `pidAlive`/`commandLineAlive`、nested-finally 清理上报）已实现并 Linux 五 E2E + bridge 全绿、已独立通过；T17 最终独立复审待验收） |
