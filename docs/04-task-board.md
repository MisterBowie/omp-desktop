...
| T20 | M5 | Plan/Goal 和高权限工具能力门 | T17-T19 | 不借用不适用的原权限/模式语义 | **进行中 / 受阻**（T20-R3B provider/bridge 层审计已完成，分支 `codex/m5-r3-provider-gate`：确定性 RED 实验（真实 Cursor exec 派发器 + 真实桥接）证明混合批次兄弟调用在裁决前执行；方案 A「延迟到 message_end」自锁死、方案 B「全 loop 路由」原生帧无请求侧开关只能伪造 `Tool not available` = 产品限制；**R3 仍阻塞、T20-B/C/D 未开始**，证据 `docs/validation/M5-omp-transition-patch.md` §9。T20-A 审计与契约经三轮复审返修（R1-R4、F1-F7、F8-F12）+ 一轮纯文档澄清（F13/F14），证据 `docs/validation/M5-plan-goal-capability-gates.md`；**T20-R3A 补丁轮已完成返修**：patch level `d49918f+omp-desktop.1` 提供 loop 层收敛（全 toolCall 计数、sole 会话关闭投机、批次拒绝、terminate 透传）与 patch/应用脚本入口，但复审 F3 确认 Cursor exec channel 的预执行副作用无法在 loop 层阻止/撤销，**R3 仍为硬阻塞、T20-B 不得开始**（「已解除」声明已撤回），证据 `docs/validation/M5-omp-transition-patch.md`（脚本 suite 本机 Linux 与 macOS 均 24/24，macOS 由独立复审在 `47d0258` 实跑确认）；T20-B/C/D 未实现，T20 未完成） |
