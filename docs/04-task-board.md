...
当前交付阶段：M5/T20 进行中——**T20-A（Plan/Goal 能力审计、基线红灯诊断与最小实现契约）已完成**（分支 `codex/m5-plan-goal-capability-gates`，基线 `2ca256520980aa80bb812730fbad132e9e95f503`；证据见 `docs/validation/M5-plan-goal-capability-gates.md`；**T20-B/C/D 待开始，T20 未完成**）。此前 M5/T19（MCP、规则、技能、记忆及插件分类适配）已完成——**T19-A（运行时能力源隔离、可信网关加载与能力所有权契约）已实现并验证**（分支 `codex/m5-capability-sources`，基线 `ff4c1efed63475b9f552ace8a3f28f69b317e176`；证据见 `docs/validation/M5-capability-sources.md`、`app/docs/adr/0304-omp-capability-source-ownership.md`）；**T19-B（host-tool RPC：桌面/插件 MCP 与插件 Agent Tools 经单一宿主边界恰一次暴露）已实现并验证**（分支 `codex/m5-host-tool-rpc`，基线 `93ee82b61bd8191722db199e0fb8f1c74ddfaa12`；证据见 `docs/validation/M5-host-tool-rpc.md`）；**T19-C（桌面技能/记忆/插件用户路径）已实现并验证**（分支 `codex/m5-capability-user-paths`，基线 `df49b84f8246a83c4ed5cb1e2a3e0c72ee88e538`；证据见 `docs/validation/M5-capability-user-paths.md`；ADR 0304 §5 追加）。T18 已通过独立复审并验收（验收产品基线 `a35da2f87a745fbedd8f5beb46368607c2f6b72e`，分支 `codex/m5-tool-results`；证据见 `docs/validation/M5-tool-results.md`）。T17 已通过最终独立复审并验收（验收代码基线 `ab07a888d6afb916561b80e5661ed27aa6be612a`，分支 `codex/m5-subagents`；证据见 `docs/validation/M5-subagents.md`、`app/docs/adr/0303-omp-subagent-surfacing.md`）。下一实现任务入口：M5/T20-B（按 `docs/validation/M5-plan-goal-capability-gates.md` §6-§7 契约实现）。历轮独立复审返修的历史记录见 `docs/validation/M5-subagents.md` §0.1-§0.14。
...
| T20 | M5 | Plan/Goal 和高权限工具能力门 | T17-T19 | 不借用不适用的原权限/模式语义 | **进行中**（T20-A 已完成：能力审计+基线红灯诊断+最小实现契约，证据 `docs/validation/M5-plan-goal-capability-gates.md`；T20-B/C/D 待开始，T20 未完成） |
