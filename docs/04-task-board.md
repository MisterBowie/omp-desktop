...
| T17 | M5 | OMP 子代理面板与编排归属 | T16 | 父子事件、查看、停止、恢复边界 | 已完成（`docs/validation/M5-subagents.md`；ADR 0303；`subagent-frames.ts`/`subagents.ts` 严格校验与每子代理转换、`get_subagents` reconcile、`get_subagent_messages` 上界读取、`stopSubagent` 恒拒、父 stop 回收 detached 子代理、详情桥接线；`subagentEvents` 能力开放；独立复审 R1-R8 与第二轮 S1-S4 返修已提交） |
